	...
	public class Unit : MonoBehaviour {
		...
		void Start ()
		{
			...
			if (FreeUnit == false) {
                ...
                //add the unit in the enemy list of all other faction managers:
                for (int i = 0; i < GameMgr.Factions.Count; i++)
                {
                    GameMgr.Factions[i].FactionMgr.EnemyUnits.Add(this);
                }
            }
		...
		public void RemoveFromFaction ()
		{
            ...
            if(FreeUnit == true)
                //remove the unit from the enemy list of all other faction managers:
                for (int i = 0; i < GameMgr.Factions.Count; i++)
                {
                    GameMgr.Factions[i].FactionMgr.EnemyUnits.Remove(this);
                }

            //But if this is a faction unit and it has a valid faction manager
            else if (FactionMgr != null) {
