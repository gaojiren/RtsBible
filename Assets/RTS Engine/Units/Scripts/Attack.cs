    ...
    public class Attack : MonoBehaviour
    {
        ...
        bool SearchForTargetToAttack()
        {
            ...
            if (AttackRangeFromCenter == true && AttackRangeCenter != null)
            {
                ...
                searchSize = AttackRangeCenter.GetComponent<Border>().Size; //and the search size is the whole city border size:
            }

            Collider[] collidersInRange = Physics.OverlapSphere(searchFrom, searchSize); //search for the colliders inside the specified range
            float minDistance = 0.0f; //we'll pick the closest target unit to attack, this will hold the minimum distance between the attacker & potential target
            Unit potentialTarget = null; //this will hold the potential target unit to attack

            for (int i = 0; i < collidersInRange.Length; i++)
            {
                //get the Unit component of the collider in range gameobject.
                Unit unitInRange = collidersInRange[i].gameObject.GetComponent<Unit>();
                if (unitInRange != null && unitInRange != UnitMgr) //if this is a valid component and it's not refering to the attacker (if it's a unit)
                {
                    //make sure that the Unit component is enabled, the unit isn't dead and that the attacker's faction can attack the unit in range's faction & last that the unit isn't invisible
                    if (unitInRange.enabled == true && CanAttackFaction(unitInRange.FactionID) && unitInRange.Dead == false && unitInRange.IsInvisible == false)
                    {
                        //if this attacker can specifically attack the target.
                        if (AttackMgr.CanAttackTarget(this, collidersInRange[i].gameObject))
                        {
                            //if we still don't have a potential target or we found a closer one:
                            float currDistance = Vector3.Distance(transform.position, unitInRange.transform.position);
                            if (potentialTarget == null || minDistance > currDistance)
                            {
                                //update potential target
                                potentialTarget = unitInRange;
            ...
            //if we haven't found a potential target
            if (potentialTarget == null)
                return false; //failure.

            //if we did find it, set it as the target
            if (AttackerType == AttackerTypes.Unit) //if the attacker is a unit
            {
                MvtMgr.LaunchAttack(UnitMgr, potentialTarget.gameObject, MovementManager.AttackModes.None);
            }
            else //if this is a building
            {
                SetAttackTarget(potentialTarget.gameObject);
            }
