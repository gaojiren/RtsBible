...
namespace RTSEngine
{
    ...
    public enum InputTargetMode { None, Self, Unit, Building, Resource, Faction, Attack, Portal, Upgrade }; //this the type of the input action that will be sent
        ...
        public void LaunchCommand(InputVars Command)
        {
            ...
            { //spawning the faction capital building
                ...
                if (GameMgr.Factions[Command.FactionID].TypeInfo != null) //valid faction type info is required
                {
                    ...
                    { //if the faction belongs to a certain type
                        ...
                        capitalBuilding = BuildingManager.CreatePlacedInstanceLocal(GameMgr.Factions[Command.FactionID].TypeInfo.capitalBuilding, Command.InitialPos, null, Command.FactionID, true);
            ...
            { //object creation.
                 ...
                 * */
                    ...
                    newInstance = BuildingManager.CreatePlacedInstanceLocal(prefab.GetComponent<Building>(), Command.InitialPos, buildingCenter, Command.FactionID, placedByDefault).gameObject;
            ...
            { //if the source mode is a building
                ...
                if (SourceBuilding)
                {
                    if (Command.TargetMode == (byte)InputTargetMode.Upgrade)
                    {
                        //upgrade the building's instance.
                        UpgradeManager.instance.UpgradeBuildingInstance(SourceBuilding, SourceBuilding.GetComponent<BuildingUpgrade>().target, Command.TargetID);
                    }
                    else if (Command.TargetMode != (byte)InputTargetMode.None)
                    { //if there's a target object:
