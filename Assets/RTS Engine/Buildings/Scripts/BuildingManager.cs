...
namespace RTSEngine
{
    public class BuildingManager : MonoBehaviour
    {
        ...
        public static void CreatePlacedInstance(Building buildingPrefab, Vector3 placementPosition, Border buildingCenter, int factionID, bool placedByDefault = false)
        {
            if (GameManager.MultiplayerGame == false)
            { //if it's a single player game.
                BuildingManager.CreatePlacedInstanceLocal(buildingPrefab, placementPosition, buildingCenter, factionID, placedByDefault); //place the building
            }
            else
            { //in case it's a multiplayer game:
                ...
                newInputAction.Source = buildingPrefab.gameObject;
                newInputAction.InitialPos = placementPosition;
                newInputAction.Target = buildingCenter.gameObject;
                newInputAction.Value = (placedByDefault == true) ? 1 : 0;
        ...
        //creates an instance of a building that is instantly placed:
        public static Building CreatePlacedInstanceLocal(Building buildingPrefab, Vector3 placementPosition, Border buildingCenter, int factionID, bool placedByDefault = false)
        {
