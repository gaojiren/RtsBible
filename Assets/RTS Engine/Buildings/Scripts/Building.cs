...
namespace RTSEngine
{
    public class Building : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            //if this is a free building then mark it as placed as factions are not allowed to place free buildings so they need to be placed by default
            if (FreeBuilding == true)
            {
                FactionID = -1;
                if (PlacedByDefault == false) //if the free building is not placed by default
                    PlaceBuilding(FactionID); //place it
            }
        ...
        public void CheckBuildingPos()
        {
            ...
            if (FactionID == GameManager.PlayerFactionID && FogOfWar.GetFogOfWarTeam(FactionID).IsInFog(transform.position, 0.5f))
            {
